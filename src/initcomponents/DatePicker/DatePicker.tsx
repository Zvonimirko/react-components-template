...
export const DatePicker = ({
  ...
  return (
    <div>
      ...
      <div
        className={twMerge(
          clsx('relative mt-1 rounded-md shadow-sm', { 'w-full': widthFull }, { 'w-[180px]': !widthFull }, containerClass),
        )}
      >
        <div className={twMerge(clsx('pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3', leadingIconClass))}>
          {leadingIcon}
        </div>
        <ReactDatePicker
          ref={datepickerRef}
          ...
          className={twMerge(
            clsx(
              ...
              { 'w-full': widthFull, 'pl-10': leadingIcon, 'pr-10': trailingIcon },
              {
                [`!border-red !text-red !focus:border-red !focus:outline-none !focus:ring-red-500 ${errorClass}`]: error,
              },
              className,
            ),
          )}
          ...
          dayClassName={(date) => {
            ...
            let dayClass = '!w-10 !h-10 sm:!w-11 sm:!h-11 !p-1 !m-0 !text-sm !leading-8 sm:!leading-9 !rounded-2xl';
            let overrideClass = '';
          ...
          weekDayClassName={(date) => {
            ...
            return twMerge(clsx('!w-10 sm:!w-11 !m-0 !mb-4 !text-sm !leading-9', weekDaysClassName));
          }}
          monthClassName={(date) => {
            if (monthClassName) {
            ...
            currentDate.setHours(0, 0, 0, 0);
            // if selected month
            if (currentValue && currentValue.getMonth() === monthClassNameIndex) {
              monthClass += ' ';
        ...
        <div
          className={twMerge(clsx('pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3', trailingIconClass))}
        >
          {trailingIcon}
        </div>
      </div>
