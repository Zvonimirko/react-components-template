...
export const Input = ({
  ...
  return (
    <div>
      ...
      >
        <div className={twMerge(clsx('absolute inset-y-0 left-0 flex items-center pl-3', leadingIconClass))}>{leadingIcon}</div>
        <input
          type="text"
          className={twMerge(
            clsx(
          ...
          ref={ref}
        />
        <div className={twMerge(clsx('absolute inset-y-0 right-0 flex items-center pr-3', trailingIconClass))}>
          {trailingIcon}
        </div>
      </div>
      {errorMessage && <p className={twMerge(clsx('mt-1 text-sm text-red-600', errorMessageClass))}>{errorMessage}</p>}
      {successMessage && <p className={twMerge(clsx('mt-1 text-sm text-red-600', successMessageClass))}>{successMessage}</p>}
    </div>
  );
};
